...
function HArrow({ color = P.textMut }) {
  return (
...
function VArrow({ color = P.textMut }) {
  return (
...
function Tag({ children, bg, fg, style: extra }) {
  return (
    <span style={{
...
function Header() {
  return (
    <header style={{ textAlign: 'center', marginBottom: 48 }}>
      ...
      <p style={{ fontSize: 14, color: P.textSec, margin: '12px auto 0', maxWidth: 680, lineHeight: 1.6 }}>
        MLOps-inspired CI/CD for <a href="https://openclaw.ai" target="_blank" rel="noopener noreferrer" style={{ color: P.cyan, textDecoration: 'none' }}>OpenClaw</a> agent
        fleets. Prescriptive, versioned system images provide a managed runtime. Portable data disks
        carry agent identity, workspace, and state across VM replacements and image upgrades. Deploy a
        fully equipped, desktop-running claw on Azure in one command. Upgrade it without losing state
        in another.
      </p>
      <a
        ...
        style={{ fontSize: 12, color: P.blue, marginTop: 12, display: 'inline-block', ...mono }}
      >
        github.com/logan-robbins/openclawps
      </a>
      <br />
      <a
        href="https://linkedin.com/in/loganrobbins"
        target="_blank"
        rel="noopener noreferrer"
        style={{ fontSize: 12, color: P.blue, display: 'inline-block', ...mono }}
      >
...
function Prerequisites() {
  const cliTools = [
    { label: 'Azure CLI', detail: 'az login — any subscription', cmd: 'az login', color: P.blue },
    { label: 'envsubst', detail: 'Renders cloud-init templates', cmd: 'brew install gettext', color: P.green },
    { label: 'sshpass', detail: 'Deploy-time SSH automation', cmd: 'brew install sshpass', color: P.cyan },
  ];
  const config = [
    { label: 'Telegram Bot Token', detail: 'Unique per claw — create via @BotFather', color: P.cyan },
    { label: 'LLM API Key', detail: 'Any supported provider: xAI, OpenAI, or Anthropic', color: P.amber },
  ];

  return (
    <section style={{ background: P.surface, border: `1px solid ${P.border}`, borderRadius: 12, padding: 32, borderTop: `3px solid ${P.amber}` }}>
      <SectionLabel color={P.amber}>Prerequisites</SectionLabel>

      {/* CLI Tools */}
      <div style={{ fontSize: 11, fontWeight: 600, color: P.textMut, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>
        CLI Tools
      </div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 20 }}>
        {cliTools.map((t, i) => (
          <React.Fragment key={t.label}>
            <div style={{
              background: P.card, border: `1px solid ${t.color}30`, borderRadius: 8,
              padding: '14px 18px', minWidth: 180, textAlign: 'center',
            }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 4 }}>
                <Dot color={t.color} />
                <span style={{ fontSize: 13, fontWeight: 600, color: P.text }}>{t.label}</span>
              </div>
              <div style={{ fontSize: 11, color: P.textSec, marginBottom: 6 }}>{t.detail}</div>
              <Tag bg={`${t.color}20`} fg={t.color}>{t.cmd}</Tag>
            </div>
            {i < cliTools.length - 1 && <HArrow color={P.textMut} />}
          </React.Fragment>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, flexWrap: 'wrap' }}>
        {config.map((c, i) => (
          <React.Fragment key={c.label}>
            <div style={{
              background: P.card, border: `1px solid ${c.color}30`, borderRadius: 8,
              padding: '14px 18px', minWidth: 220, textAlign: 'center',
            }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 4 }}>
                <Dot color={c.color} />
                <span style={{ fontSize: 13, fontWeight: 600, color: P.text }}>{c.label}</span>
              </div>
              <div style={{ fontSize: 11, color: P.textSec }}>{c.detail}</div>
            </div>
            {i < config.length - 1 && <HArrow color={P.amber} />}
          </React.Fragment>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <div style={{
          background: P.card, border: `1px solid ${P.amber}30`, borderRadius: 8,
          padding: '12px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6,
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Tag bg={P.amberDim} fg={P.amber}>cp .env.template .env</Tag>
          </div>
          <span style={{ fontSize: 11, color: P.textSec, textAlign: 'center', maxWidth: 400, lineHeight: 1.4 }}>
            Full configuration reference in{' '}
            <a href="https://github.com/logan-robbins/openclawps/blob/main/.env.template" target="_blank" rel="noopener noreferrer" style={{ color: P.amber, textDecoration: 'none' }}>.env.template</a>
            {' '}and{' '}
            <a href="https://github.com/logan-robbins/openclawps/blob/main/README.md" target="_blank" rel="noopener noreferrer" style={{ color: P.amber, textDecoration: 'none' }}>README</a>
            . Each claw gets its own .env — same image, different config, different claw.
          </span>
        </div>
      </div>
    </section>
...
function Pipeline() {
  const phases = [
    { n: 1, cmd: 'scratch', title: 'Build from Source', desc: 'Stock Ubuntu 24.04, cloud-init full install — xfce4 desktop, Chrome, OpenClaw, Claude Code (~10 min)',
      artifacts: ['cloud-init.yaml', 'Full graphical desktop on :0 with VNC'], mlops: 'Train from scratch', color: P.blue },
    { n: 2, cmd: 'bake x.y.z', title: 'Capture Image', desc: 'Generalize VM into immutable versioned image, push to Azure Compute Gallery',
      artifacts: ['gallery/claw-base/x.y.z', 'waagent deprovision, secrets stripped'], mlops: 'Registry push', color: P.green },
    { n: 3, cmd: '(default)', title: 'Stamp Out a Claw', desc: 'Golden image + fresh data disk — fleet-friendly, same image different .env (~2 min)',
      artifacts: ['cloud-init-slim.yaml (secrets only)', 'Data disk auto-partitioned + seeded'], mlops: 'Inference deploy', color: P.cyan },
    { n: 4, cmd: 'upgrade', title: 'Upgrade in Place', desc: 'Swap VM to new image version, same data disk — identity, workspace, memory, credentials preserved',
      artifacts: ['Detach data disk, delete VM, recreate from new image, reattach', 'NIC + public IP reused, migration scripts run automatically'], mlops: 'Rolling update', color: P.purple },
  ];
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <tbody>
              {([
                ...
                ['Exec sandbox', <span>off — containment at infra boundary, not inside guest</span>],
